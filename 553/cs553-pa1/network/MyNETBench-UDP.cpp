...
#define reportPath "/exports/home/schatterjee/reports/report-udp.txt"
#define sharedPath "/exports/home/schatterjee/ips/udpserverip"
...
void *clientWorker(void *threadarg) {
    ...
    while (*totatlRecv < 100 * oneGBtoByte) {
        try{
            numOfBytesSent = sendto(sockfd, dataToSend, chunkSize, 0, (struct sockaddr *) &serv_addr, servlen);
        } catch (int e){
            cout << e << endl;
        }
    ...
    const char end = 'z';
    sendto(sockfd, &end, 1, 0, (struct sockaddr *) &serv_addr, servlen);
    close(sockfd);
...
void *serverWorker(void *threadarg) {
    ...
    long numOfBytesRead;
    while (*totatlRecv < 100 * oneGBtoByte || *buffer == 'z') {
        try {
            numOfBytesRead = recvfrom(sockfd, buffer, chunkSize, 0, (struct sockaddr *) &remaddr, &addrlen);
        } catch(int e){
            cout << e;
        }
        if (numOfBytesRead > 0) {
...
int main(int argc, char *argv[]) {
    ...
    string report = "\n  Threads " + to_string(NUM_THREADS) +
                    ...
                    ", Throughput in Mbps: " +  to_string(totalRecv*8.0/(exec_t*1000*1000.0)) +
